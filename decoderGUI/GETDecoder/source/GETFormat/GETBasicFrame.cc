...
void GETBasicFrame::Read(ifstream &stream) {
  ...
  if (GetFrameType() == GETFRAMEBASICTYPE1) {
    ...
    uint8_t data[2];
    std::cout << "NItems: " << GetNItems() << " ItemSize: " << GetItemSize() << std::endl;
    std::cout << "EventTime: " << GetEventTime() << " EventID: " << GetEventID() << std::endl;
    for (Int_t iItem = 0; iItem < GetNItems(); iItem++) {
      stream.read((Char_t *) data, GetItemSize());

      UShort_t item = CorrectEndianness(data, 2);
      if (debug)
	std::cout << "Item: " << std::hex << item << std::endl;
      ...
      if (debug){
	std::cout << agetIdx << " " << chIdx << " " << tbIdx << " " << sample << std::endl;
      }
...
void GETBasicFrame::Read(istream &stream) {
  Clear();
